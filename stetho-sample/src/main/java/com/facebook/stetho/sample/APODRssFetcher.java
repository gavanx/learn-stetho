...
public class APODRssFetcher {
    ...
    public void fetchAndStore() {
        Networker.HttpRequest request = Networker.HttpRequest.newBuilder().friendlyName("APOD RSS").method(Networker.HttpMethod.GET).url
            (APOD_RSS_URL).build();
    ...
    private void fetchLinkPage(String linkUrl, PageScrapedCallback callback) {
        ...
        Networker.HttpRequest request = Networker.HttpRequest.newBuilder().friendlyName("fetchLinkPage").method(Networker.HttpMethod.GET).url
            (linkUrl).build();
    ...
    private interface PageScrapedCallback {
        /**
         * @param imageUrls Image URLs that were scraped or null if the page could not be fetched or parsed.
         */
        public void onPageScraped(@Nullable List<String> imageUrls);
    }
}
