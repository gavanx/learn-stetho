    ...
    private void initializeStetho(final Context context) {
        ...
        Stetho.initialize(Stetho.newInitializerBuilder(context).enableDumpapp(new DumperPluginsProvider() {
            ...
            public Iterable<DumperPlugin> get() {
                return new Stetho.DefaultDumperPluginsBuilder(context).provide(new HelloWorldDumperPlugin()).provide(new APODDumperPlugin(context
                    .getContentResolver())).finish();
            }
    ...
    private static class ExtInspectorModulesProvider implements InspectorModulesProvider {
        ...
        private ContentProviderDatabaseDriver createContentProviderDatabaseDriver(Context context) {
            ...
            ContentProviderSchema eventsSchema = new ContentProviderSchema.Builder().table(new Table.Builder().uri(CalendarContract.Events
                ...
                .DESCRIPTION, CalendarContract.Events.ACCOUNT_NAME, CalendarContract.Events.DTSTART, CalendarContract.Events.DTEND,
                CalendarContract.Events.CALENDAR_ID,}).build()).build();
