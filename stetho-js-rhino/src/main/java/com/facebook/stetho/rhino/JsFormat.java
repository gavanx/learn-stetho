 ...
 * This happens because %d expects an int/Integer but in JavaScript numbers are floats! </p>
 *
 * <p>See <a href="https://developer.chrome.com/devtools/docs/console-api#consolelogobject-object">Console API</a>.</p>
 */
class JsFormat {
    /**
     * Format specifier pattern. <p/> <code>%[argument_index$][flags][width][.precision]conversion</code>
     */
    private static final Pattern FORMAT_SPECIFIER_PATTERN = Pattern.compile("^%" + "([0-9]+ [$])?"  // Index
            + "([0-9]+)?"      // Width
            + "([.] [0-9]+)?"  // Precision
            + "([difs])",      // Conversion
        Pattern.COMMENTS);
    ...
    /**
     * Simple wrapper around a char[]. New versions of java make a full copy of the string when doing substring(). With this class we avoid the copies
    ...
    private static class ArrayCharSequence implements CharSequence {
        ...
        private
        @NonNull
        CharSequence substring(int start) {
            return new ArrayCharSequence(array, this.start + start, this.start + end);
        }
    ...
    /**
     * Takes the arguments that console.log() would use, parses them and returns the final string to output.
     *
    ...
    static
    @NonNull
    String parse(@NonNull Object... args) {
