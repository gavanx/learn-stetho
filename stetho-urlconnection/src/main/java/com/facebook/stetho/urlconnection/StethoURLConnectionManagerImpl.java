...
/**
 * Isolated implementation class to allow us to escape the verifier if Stetho is not present.  This is done for convenience so that {@link
 * StethoURLConnectionManager} hooks can be left in a release build without any significant overhead either at runtime or in the compiled APK.
 */
class StethoURLConnectionManagerImpl {
    ...
    public void postConnect() throws IOException {
        ...
        if (isStethoActive()) {
            ...
            mStethoHook.responseHeadersReceived(new URLConnectionInspectorResponse(getStethoRequestId(), mConnection));
        }
    }
    ...
    public InputStream interpretResponseStream(@Nullable InputStream responseStream) {
        ...
        if (isStethoActive()) {
            ...
            responseStream = mStethoHook.interpretResponseStream(getStethoRequestId(), mConnection.getHeaderField("Content-Type"), mConnection
                .getHeaderField("Content-Encoding"), responseStream, new DefaultResponseHandler(mStethoHook, getStethoRequestId()));
        }
