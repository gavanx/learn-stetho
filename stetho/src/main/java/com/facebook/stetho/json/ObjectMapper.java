...
/**
 * This class is a lightweight version of Jackson's ObjectMapper. It is designed to have a minimal subset of the functionality required for stetho.
 * <p> It would be awesome if there were a lightweight library that supported converting between arbitrary {@link Object} and {@link JSONObject}
 * representations. <p> Admittedly the other approach would be to use an Annotation Processor to create static conversion functions that discover
 * something like a {@link JsonProperty} and create a function at compile time however since this is just being used for a simple debug utility and
 * Kit-Kat caches the results of reflection this class is sufficient for stethos needs.
 */
public class ObjectMapper {
    ...
    /**
     * Support mapping between arbitrary classes and {@link JSONObject}. <note> It is possible for a {@link Throwable} to be propagated out of this
     * class if there is an {@link InvocationTargetException}. </note>
     *
     * @throws IllegalArgumentException when there is an error converting. One of either {@code fromValue.getClass()} or {@code toValueType} must be
     *                                  {@link JSONObject}.
     */
    ...
    private <T> T _convertFromJSONObject(JSONObject jsonObject, Class<T> type) throws NoSuchMethodException, IllegalAccessException,
    ...
    /**
     * In this case we know that there is an {@link Enum} decorated with {@link JsonValue}. This means that we need to iterate through all of the
     * values of the {@link Enum} returned by the given {@link Method} to check the given value.
     */
