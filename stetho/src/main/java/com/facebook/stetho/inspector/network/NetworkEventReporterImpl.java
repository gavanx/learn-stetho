...
/**
 * Implementation of {@link NetworkEventReporter} which allows callers to inform the Stetho system of network traffic.  Callers can safely eagerly
 * access this class and store a reference if they wish.  When WebKit Inspector clients are connected, the internal implementation will be
 * automatically wired up to them.
 */
public class NetworkEventReporterImpl implements NetworkEventReporter {
    ...
    /**
     * Static accessor allowing callers to easily hook into the WebKit Inspector system without creating dependencies on the main Stetho
    ...
    public void requestWillBeSent(InspectorRequest request) {
        ...
        if (peerManager != null) {
            ...
            initiatorJSON.stackTrace.add(new Console.CallFrame(requestFriendlyName, requestFriendlyName, requestPriority != null ? requestPriority
                : 0 /* lineNumber */, 0 /* columnNumber */));
    ...
    private static Page.ResourceType determineResourceType(AsyncPrettyPrinter asyncPrettyPrinter, String contentType, ResourceTypeHelper
        resourceTypeHelper) {
        if (asyncPrettyPrinter != null) {
    ...
    @Override
    public InputStream interpretResponseStream(String requestId, @Nullable String contentType, @Nullable String contentEncoding, @Nullable
    InputStream availableInputStream, ResponseHandler responseHandler) {
        NetworkPeerManager peerManager = getPeerManagerIfEnabled();
        if (peerManager != null) {
            ...
            try {
                ...
                return DecompressionHelper.teeInputWithDecompression(peerManager, requestId, availableInputStream, fileOutputStream,
                    contentEncoding, responseHandler);
            } catch (IOException e) {
                CLog.writeToConsole(peerManager, Console.MessageLevel.ERROR, Console.MessageSource.NETWORK, "Error writing response body data for " +
                    "request #" + requestId);
            }
        }
