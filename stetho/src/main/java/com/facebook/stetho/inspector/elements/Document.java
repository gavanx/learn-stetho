...
public final class Document extends ThreadBoundProxy {
    ...
    public
    @Nullable
    NodeDescriptor getNodeDescriptor(Object element) {
        verifyThreadAccess();
    ...
    public
    @Nullable
    Integer getNodeIdForElement(Object element) {
    ...
    public
    @Nullable
    Object getElementForNodeId(int id) {
        // We don't actually call verifyThreadAccess() for performance.
        //verifyThreadAccess();
        ...
        while (!mCachedUpdateQueue.isEmpty()) {
            ...
            for (int i = 0, size = childrenAccumulator.size(); i < size; ++i) {
                ...
                if (child != null) {
                    ...
                    // app.
                    LogUtil.e("%s.getChildren() emitted a null child at position %s for element %s", descriptor.getClass().getName(), Integer
                        .toString(i), element);
    ...
    private void applyDocumentUpdate(final ShadowDocument.Update docUpdate) {
        ...
        docUpdate.getChangedElements(new Accumulator<Object>() {
            ...
            public void store(Object element) {
                ...
                final ElementInfo oldElementInfo = mShadowDocument.getElementInfo(element);
                final ElementInfo newElementInfo = docUpdate.getElementInfo(element);
                final List<Object> oldChildren = (oldElementInfo != null) ? oldElementInfo.children : Collections.emptyList();
    ...
    /**
     * A private implementation of {@link List} that transmits our changes to our listener (and, ultimately, to the DevTools client).
     */
    private final class ChildEventingList extends ArrayList<Object> {
        ...
        public void acquire(Object parentElement, DocumentView documentView) {
            ...
            mParentNodeId = (mParentElement == null) ? -1 : mObjectIdMapper.getIdForObject(mParentElement);
        ...
        public void addWithEvent(int index, Object element, Accumulator<Object> insertedElements) {
            ...
            int previousNodeId = (previousElement == null) ? -1 : mObjectIdMapper.getIdForObject(previousElement);

            add(index, element);

            mUpdateListeners.onChildNodeInserted(mDocumentView, element, mParentNodeId, previousNodeId, insertedElements);
        }
    ...
    public interface UpdateListener {
        ...
        void onChildNodeInserted(DocumentView view, Object element, int parentNodeId, int previousNodeId, Accumulator<Object> insertedItems);
    }
