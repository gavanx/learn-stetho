...
/**
 * {@link InputStream} that caches the data as the data is read, and writes them to the given {@link OutputStream}. This also guarantees that we will
 * attempt to reach EOF on the {@link InputStream} passing all data to the {@link OutputStream}. This is done to allow us to guarantee all responses
 * are represented in the webkit inspector.
 */
// @VisibleForTest
public final class ResponseHandlingInputStream extends FilterInputStream {
     ...
     * @param decompressedCounter Optional decompressing counting output stream which can be queried after each write to determine the number of
     *                            decompressed bytes yielded.  Used to implement {@link ResponseHandler#onReadDecoded(int)}.
     * @param networkPeerManager  A peer manager which is used to log internal errors to the Inspector console.
     * @param responseHandler     Special interface to intercept read events before they are sent to peers via {@link NetworkEventReporter} methods.
     */
    public ResponseHandlingInputStream(InputStream inputStream, String requestId, OutputStream outputStream, @Nullable CountingOutputStream
        decompressedCounter, ChromePeerManager networkPeerManager, ResponseHandler responseHandler) {
        super(inputStream);
    ...
    public void close() throws IOException {
        try {
            ...
            if (bytesRead > 0) {
                CLog.writeToConsole(mNetworkPeerManager, Console.MessageLevel.ERROR, Console.MessageSource.NETWORK, "There were " + String.valueOf
                    (bytesRead) + " bytes that were not consumed while " + "processing request " + mRequestId);
            }
        } finally {
            super.close();
            closeOutputStreamQuietly();
    ...
    private synchronized void closeOutputStreamQuietly() {
        if (!mClosed) {
            try {
                ...
                CLog.writeToConsole(mNetworkPeerManager, Console.MessageLevel.ERROR, Console.MessageSource.NETWORK, "Could not close the output " +
                    "stream" + e);
            } finally {
                mClosed = true;
            }
    ...
    /**
     * Handles reporting an {@link IOException}. We do this so we can centralize the logic while still maintaining the ability of the catch clause to
     * throw.
     */
    ...
    /**
     * Writes the byte to all the output streams. If we get an exception when writing to any of the streams, we close all the streams, and then
     * propagate the first exception that occurred when writing.
     */
    ...
    private void handleIOExceptionWritingToStream(IOException e) {
        CLog.writeToConsole(mNetworkPeerManager, Console.MessageLevel.ERROR, Console.MessageSource.NETWORK, "Could not write response body to the stream " + e);
