...
public class SqliteDatabaseDriver extends Database.DatabaseDriver {
    ...
    /**
     * Constructs the object with a {@link DatabaseFilesProvider} that supplies the database files from {@link Context#databaseList()}.
     *
     * @param context the context
     * @deprecated use the other {@linkplain SqliteDatabaseDriver#SqliteDatabaseDriver(Context, DatabaseFilesProvider) constructor} and pass in the
     * {@linkplain DefaultDatabaseFilesProvider default provider}.
     */
    @Deprecated
    ...
    /**
     * Attempt to smartly eliminate uninteresting shadow databases such as -journal and -uid.  Note that this only removes the database if it is true
     * that it shadows another database lacking the uninteresting suffix.
     *
    ...
    private SQLiteDatabase openDatabase(String databaseName) throws SQLiteException {
        ...
        return SQLiteDatabase.openDatabase(databaseFile.getAbsolutePath(), null /* cursorFactory */, SQLiteDatabase.OPEN_READWRITE);
    }
}
