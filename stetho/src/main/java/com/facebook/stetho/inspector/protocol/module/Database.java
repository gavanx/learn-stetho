...
public class Database implements ChromeDevtoolsDomain {
     ...
     * number that I think folks will actually need in practice. <p> Note that when this limit is exceeded, a dummy row will be introduced that
     * indicates truncation occurred.
     */
    private static final int MAX_EXECUTE_RESULTS = 250;

    /**
     * Maximum length of a BLOB field before we stop trying to interpret it and just return {@link #UNKNOWN_BLOB_LABEL}
     */
    ...
    public JsonRpcResult getDatabaseTableNames(JsonRpcPeer peer, JSONObject params) throws JsonRpcException {
        ...
        try {
            ...
            throw new JsonRpcException(new JsonRpcError(JsonRpcError.ErrorCode.INVALID_REQUEST, e.toString(), null /* data */));
        }
    }
    ...
    private DatabaseDriver getDatabasePeer(String databaseId) {
        for (DatabaseDriver databaseDriver : mDatabaseDrivers) {
            ...
            if (databaseNames != null && databaseNames.contains(databaseId)) return databaseDriver;
        }
        return null;
    }
     ...
     * @return List of Java primitives matching the value type of each column, converted to strings.
     */
    ...
    public static abstract class DatabaseDriver {
        ...
        public abstract ExecuteSQLResponse executeSQL(String databaseName, String query, ExecuteResultHandler<ExecuteSQLResponse> handler) throws SQLiteException;
