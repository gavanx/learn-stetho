...
public class MethodDispatcher {
    ...
    public JSONObject dispatch(JsonRpcPeer peer, String methodName, @Nullable JSONObject params) throws JsonRpcException {
        ...
        if (dispatchHelper == null) {
            throw new JsonRpcException(new JsonRpcError(JsonRpcError.ErrorCode.METHOD_NOT_FOUND, "Not implemented: " + methodName, null /* data */));
        }
        try {
            ...
            throw new JsonRpcException(new JsonRpcError(JsonRpcError.ErrorCode.INTERNAL_ERROR, e.toString(), null /* data */));
        }
    }
    ...
    private static class MethodDispatchHelper {
        ...
        public JSONObject invoke(JsonRpcPeer peer, @Nullable JSONObject params) throws InvocationTargetException, IllegalAccessException,
    ...
    /**
     * Determines if the method is a {@link ChromeDevtoolsMethod}, and validates accordingly if it is.
     *
     * @throws IllegalArgumentException Thrown if it is a {@link ChromeDevtoolsMethod} but it otherwise fails to satisfy requirements.
     */
