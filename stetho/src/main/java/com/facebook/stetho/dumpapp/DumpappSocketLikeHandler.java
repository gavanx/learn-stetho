...
/**
 * Provides a kind of CLI-over-HTTP support for the ./scripts/dumpapp tool. <p> This handler accepts a list of text-based arguments to a FAB endpoint
 * and responds with a stream as furnished by the Dumper implementation on the app side.  A special "exit code" property is also returned that the
 * dumpapp tool uses to pass along the exit code of the script.
 */
public class DumpappSocketLikeHandler implements SocketLikeHandler {
    ...
    private void establishConversation(DataInputStream in) throws IOException {
        ...
        if (version != PROTOCOL_VERSION) {
            throw logAndThrowProtocolException("Expected version=" + PROTOCOL_VERSION + "; got=" + version);
        }
    }
