...
/**
 * Yes, this intentionally crashes the app.  Useful for testing crash recovery and crash reporter work flows.  Three separate exit strategies are
 * supported; see help output for more details.
 */
public class CrashDumperPlugin implements DumperPlugin {
    ...
    private void doKill(DumperContext dumpContext, Iterator<String> argsIter) throws DumpException {
        ...
        try {
            Process kill = new ProcessBuilder().command("/system/bin/kill", "-" + signal, String.valueOf(android.os.Process.myPid()))
