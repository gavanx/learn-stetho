...
/**
 * Initialization and configuration entry point for the Stetho debugging system.  Simple usage with default plugins and features enabled: <p />
 * <pre>
 *   Stetho.initializeWithDefaults(context)
 * </pre>
 * <p /> For more advanced configuration, see {@link #newInitializerBuilder(Context)} or the {@code stetho-sample} for more information.
 */
public class Stetho {
    private Stetho() {
    }
    ...
    /**
     * Construct a simple initializer helper which allows you to customize stetho behaviour with additional features, plugins, etc.  See {@link
     * DefaultDumperPluginsBuilder} and {@link DefaultInspectorModulesBuilder} for more information. <p /> For simple use cases, consider {@link
    ...
    /**
     * Start the listening server.  Most of the heavy lifting initialization is deferred until the first socket connection is received, allowing this
     * to be safely used for debug builds on even low-end hardware without noticeably affecting performance.
     */
    ...
    /**
     * Start the listening service, providing a custom initializer as per {@link #newInitializerBuilder}.
     *
    ...
    /**
     * Configuration mechanism to customize the behaviour of the standard set of inspector modules satisfying the Chrome DevTools protocol.  Note that
     * while it is still technically possible to manually control these modules, this API is strongly discouraged and will not necessarily be
    ...
    public static final class DefaultInspectorModulesBuilder {
        ...
        /**
         * Provide a custom document provider factory which can operate on the logical DOM exposed to Chrome in the Elements tab.  An Android View
         * hierarchy instance is provided by default if this method is not called. <p /> <i>Experimental.</i>  This API may be changed or removed in
         * the future.
         */
        ...
        /**
         * Provide a custom runtime REPL (read-eval-print loop) implementation for the Console tab. By default an implementation will be provided for
         * you that automatically detects the existence of {@code stetho-js-rhino} (Mozilla's Rhino engine) and uses it if available. <p /> To
         * customize the Rhino implementation, see {@code stetho-js-rhino} documentation.
         */
        ...
        /**
         * Customize the location of database files that Stetho will propogate in the UI.  Android's {@link Context#getDatabasePath} method will be
         * used by default if not overridden here.
         */
        ...
        /**
         * Extend and provide additional database drivers. Currently two database drivers are supported in this lib: <br> 1.
         * <code>SqliteDatabaseDriver</code> - Presents sqlite databases and all tables of the app.<br> 2. <code>ContentProviderDatabaseDriver</code>
         * - Configure and present content providers data.
         */
         ...
         *
         * @deprecated This fine-grained control of the devtools modules is no longer supportable given the lack of isolation of modules in the actual
         * protocol (many cross dependencies emerge when you implement more and more of the real protocol).
         */
        @Deprecated
        ...
        public Iterable<ChromeDevtoolsDomain> finish() {
            ...
            provideIfDesired(new Runtime(mRuntimeRepl != null ? mRuntimeRepl : new RhinoDetectingRuntimeReplFactory(mContext)));
            provideIfDesired(new Worker());
            if (Build.VERSION.SDK_INT >= DatabaseConstants.MIN_API_LEVEL) {
                Database database = new Database();
                database.add(new SqliteDatabaseDriver(mContext, mDatabaseFilesProvider != null ? mDatabaseFilesProvider : new
                    DefaultDatabaseFilesProvider(mContext)));
                if (mDatabaseDrivers != null) {
    ...
    /**
     * Callers can choose to subclass this directly to provide the initialization configuration or they can construct a concrete instance using {@link
    ...
    public static abstract class Initializer {
        ...
        final void start() {
            ...
            LocalSocketServer server = new LocalSocketServer("main", AddressNameHelper.createCustomAddress("_devtools_remote"), new
                LazySocketHandler(new RealSocketHandlerFactory()));
        ...
        private class RealSocketHandlerFactory implements SocketHandlerFactory {
            ...
            public SocketHandler create() {
                ...
                if (dumperPlugins != null) {
                    ...
                    socketHandler.addHandler(new ProtocolDetectingSocketHandler.ExactMagicMatcher(DumpappSocketLikeHandler.PROTOCOL_MAGIC), new
                        DumpappSocketLikeHandler(dumper));
                ...
                if (inspectorModules != null) {
                    socketHandler.addHandler(new ProtocolDetectingSocketHandler.AlwaysMatchMatcher(), new DevtoolsSocketHandler(mContext,
                        inspectorModules));
                }
    ...
    public static class InitializerBuilder {
        ...
        /**
         * Enable use of the {@code dumpapp} system.  This is an extension to Stetho which allows developers to configure custom debug endpoints as
         * tiny programs embedded inside of a larger running Android application.  Examples of this would be simple utilities to visualize and edit
         * {@link SharedPreferences} data, kick off sync or other background tasks, inject custom data temporarily into the process for
         * debugging/reproducibility, upload error reports, etc. <p> See {@code ./scripts/dumpapp} for more information on how to use this system once
         * enabled.
         *
