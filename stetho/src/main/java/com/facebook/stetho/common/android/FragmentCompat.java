...
/**
 * Compatibility abstraction which allows us to generalize access to both the support library's fragments and the built-in framework version.  Note:
 * both versions can be live at the same time in a single application and even on a single object instance. <p/> Type safety is enforced via generics
 * internal to the implementation but treated as opaque from the outside.
 */
@NotThreadSafe
    ...
    static class FragmentManagerAccessorViaReflection<FRAGMENT_MANAGER, FRAGMENT> implements FragmentManagerAccessor<FRAGMENT_MANAGER, FRAGMENT> {
        ...
        public List<FRAGMENT> getAddedFragments(FRAGMENT_MANAGER fragmentManager) {
            ...
            if (mFieldMAdded == null) {
                Field fieldMAdded = ReflectionUtil.tryGetDeclaredField(fragmentManager.getClass(), "mAdded");
            ...
            return (mFieldMAdded != null) ? (List<FRAGMENT>) ReflectionUtil.getFieldValue(mFieldMAdded, fragmentManager) : null;
        }
    }
}
