...
/**
 * Implemented by an object whose methods must be called on a specific thread. If a method is called from a disallowed thread then {@link
 * IllegalStateException} will be thrown. To marshal a call to the correct thread, you can use {@link #postAndWait(UncheckedCallable)} or {@link
 * #postAndWait(Runnable)}, both of which complete synchronously.
 */
public interface ThreadBound {
    ...
    /**
     * Synchronously executes an {@link UncheckedCallable} on the thread that this object is bound to, and returns its result.
     *
     ...
     * @throws RuntimeException if the {@link UncheckedCallable} could not be executed (the cause will be null), or if {@link
     *                          UncheckedCallable#call()} threw an exception (the cause will be the exception that it threw).
     */
     ...
     * @throws RuntimeException if the {@link Runnable} could not be executed (the cause will be null), or if {@link Runnable#run()} threw an
     *                          exception (the cause will be the exception that it threw).
     */
    void postAndWait(Runnable r);

    /**
     * Asynchronously executes a {@link Runnable} on the thread that this object is bound to after the specified delay.
     *
     * @param r           the {@link Runnable} to execute
