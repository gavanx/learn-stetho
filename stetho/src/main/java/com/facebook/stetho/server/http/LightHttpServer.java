...
/**
 * Somewhat crude but very fast HTTP server designed exclusively to handle the Chrome DevTools protocol, though sufficiently general to do other very
 * basic things. Performance is imperative here as Chrome aggressively polls Stetho asking for meta data when the discovery window is open in Chrome.
 */
public class LightHttpServer {
    ...
    /**
     * Efficient, unbuffered variation of {@link InputStreamReader} which assumes the input is always ASCII.  This is especially useful when you are
     * certain that the client and server are both mechanized and will not contain non-ASCII characters in the control messages upon which this reader
     * is applied.
     */
    ...
    /**
     * Similar in spirit to {@link HttpMessageReader} which assumes ASCII for all messages as a performance optimization.  Caller is responsible for
     * flushing the writer. <p /> Exposed publicly as a hack to support WebSocket upgrade.
     */
